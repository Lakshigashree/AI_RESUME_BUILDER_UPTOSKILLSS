...
const normalizeData = (raw) => {
  ...
  data.skills = safeArray(data.skills).map(
    s => typeof s === 'string' ? s : (s.name || "")
  );
  
  data.achievements = safeArray(data.achievements).map(
    a => typeof a === 'string' ? a : (a.title || "")
  );
    ...
    experience: hasContent("experience") && (
      ...
      >
        ...
        {safeArray(localData.experience).map((exp, i) => (
          <div 
            key={i} 
            style={editMode ? styles.editRow : { marginBottom: 16 }}
          >
            {editMode ? (
              <>
                <input 
    ...
    education: hasContent("education") && (
      ...
      >
        ...
        {safeArray(localData.education).map((edu, i) => (
          <div 
            key={i} 
            style={editMode ? styles.editRow : { marginBottom: 12 }}
          >
            {editMode ? (
              <>
                <input 
              ...
              >
                <span>
                  <strong>{renderSafe(edu.degree)}</strong>, {renderSafe(edu.institution)}
                </span>
                <span>
                  {edu.duration}
                </span>
      ...
      >
        ...
        >
          {safeArray(localData.skills).map((skill, i) => (
            <div 
              key={i} 
              style={editMode ? { border: "1px solid #ccc", padding: "2px 5px" } : styles.pill}
            >
              {editMode ? (
                <input 
                  style={{ 
    ...
    achievements: hasContent("achievements") && (
      ...
      >
        ...
        {safeArray(localData.achievements).map((ach, i) => (
          <div 
            key={i} 
            style={editMode ? styles.editRow : { marginBottom: 5 }}
          >
            {editMode ? (
              <div 
                style={{ 
