    ...
    experience: hasContent("experience") && (
      ...
      >
        ...
        {(localData.experience || []).map((exp, i) => (
          <div 
            key={i} 
            style={editMode ? styles.editRow : { marginBottom: "1.5rem" }}
          >
            {editMode ? (
              <>
                <input 
    ...
    education: hasContent("education") && (
      ...
      >
        ...
        {(localData.education || []).map((edu, i) => (
          <div 
            key={i} 
            style={editMode ? styles.editRow : { marginBottom: "1rem" }}
          >
            {editMode ? (
              <>
                <input 
    ...
    certifications: hasContent("certifications") && (
      ...
      >
        <div style={styles.sectionTitle}>
          Certifications 
          {editMode && (
            <button onClick={() => addItem("certifications", { title: "" })}>
              +
            </button>
          )}
        </div>
                ...
                {['phone', 'email', 'linkedin', 'github', 'portfolio', 'location'].map(k => (
                  <div key={k}>
                    {editMode ? (
                      ...
                      <a 
                        href={k === 'phone' ? `tel:${localData[k]}` : k === 'email' ? `mailto:${localData[k]}` : normalizeUrl(localData[k])} 
