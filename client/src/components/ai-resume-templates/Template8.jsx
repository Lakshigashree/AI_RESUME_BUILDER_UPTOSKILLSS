        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
          {safeArray(localData.skills).map((s, i) => (
            ...
            >
              {editMode ? (
                <input 
                  style={{ border: "none", background: "transparent", color: "white", width: "70px" }} 
                  value={renderSafe(s)} 
            ...
            {editMode ? (
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: "bold", fontSize: "14px" }}>
                  <span>{renderSafe(exp.title)}</span>
                  <span>{exp.date}</span>
                </div>
            ...
            {editMode ? (
              <div style={{ display: "grid", gap: "5px" }}>
                ...
                <button 
                  ...
                  style={{ color: "red", border: "none", background: "none", textAlign: "left", fontSize: "12px" }}
                >
                  Remove
                </button>
              </div>
    ...
    projects: hasContent(localData, "project", editMode) && (
      <div key="projects" style={editBoxStyle}>
        <div style={sectionTitleStyle}>
          ...
          {editMode && (
            <button 
              onClick={() => addItem("project", { title: "", description: "" })} 
              style={{ 
                fontSize: "12px", 
                border: "1px solid #ff7b25", 
                background: "none", 
          ...
          <div key={i} style={{ marginBottom: "10px" }}>
            {editMode ? (
              <div style={{ display: "grid", gap: "5px" }}>
                <input 
                  style={inputStyle} 
                  value={renderSafe(prj.title)} 
                  onChange={(e) => handleArrayUpdate("project", i, e.target.value, "title")} 
                />
                <textarea 
                  style={inputStyle} 
                  value={renderSafe(prj.description)} 
                  onChange={(e) => handleArrayUpdate("project", i, e.target.value, "description")} 
                />
                <button 
        ...
        {safeArray(localData.certifications).map((cert, i) => (
          ...
          >
            {editMode ? (
              <input 
                style={inputStyle} 
                value={renderSafe(cert.name)} 
                onChange={(e) => handleArrayUpdate("certifications", i, e.target.value, "name")} 
              />
            ) : (
              <span>• {renderSafe(cert.name)}</span>
            )}
            {editMode && (
              <button 
    ...
    achievements: hasContent(localData, "achievements", editMode) && (
      <div key="achievements" style={editBoxStyle}>
        <div style={sectionTitleStyle}>
          ...
          {editMode && (
            <button 
              onClick={() => addItem("achievements", { title: "", year: "" })}
              style={{ 
                fontSize: "12px", 
                border: "1px solid #ff7b25", 
                background: "none", 
        ...
        {safeArray(localData.achievements).map((ach, i) => (
          ...
          >
            {editMode ? (
              <input 
                style={inputStyle} 
                value={renderSafe(ach.title)} 
                onChange={(e) => handleArrayUpdate("achievements", i, e.target.value, "title")} 
              />
            ) : (
              <span>• {renderSafe(ach.title)}</span>
            )}
            {editMode && (
              <button 
                    ...
                    {editMode ? (
                      ...
                      localData[key] && (
                        key === "email" ? (
                          ...
                          <a 
                            href={normalizeUrl(localData[key])} 
                            target="_blank" 
                            rel="noreferrer" 
                            style={{ color: "inherit", textDecoration: "none" }}
                          >
