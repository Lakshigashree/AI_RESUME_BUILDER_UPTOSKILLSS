// Add enhanceSectionContent to your existing imports
const { enhanceSectionContent } = require('../services/geminiService');
...
const createResume = async(req, res) => {
    ...
    try {
        ...
        console.log("🟣 Forcing Template ID to:", safeTemplateId);
...
/**
 * Controller to handle AI enhancement requests from the Sidebar
 */
const enhanceResumeSection = async (req, res) => {
  try {
    // 1. Grab the data sent from Sidebar.jsx
    ...
    // 3. Send it to your updated Gemini service
    const enhancedContent = await enhanceSectionContent(section, content, tone);

    // 4. Send the exact JSON structure the frontend is waiting for
    res.status(200).json({
...
module.exports = {
    ...
    deleteResume,
    getUserSuggestions,
    enhanceResumeSection, // Export the new controller function
};